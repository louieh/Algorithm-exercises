...
### Booking.com Hackerrank

At Booking.com our customer service team is an important contributor to customer satisfaction. During busy times, however, there might be more calls to customer service than the number of customer service executives can manage. Fortunately, we record data on that. We've collected information about all phone calls to our call centres for the past year.

Given that our current number of customer care agents is X. Determine how many more people we would need to hire, to make sure that our customers would not have to wait during peak hours (i.e. that we don't have more phone calls than we have customer service executives).
...
Input:
 ...
 The next N lines are whitespace-separated pairs of timestamps (a timestamp is an integer that represents seconds since the epoch). On each line, the first time is the time when the call was started, and the second one is when that call ended.

Output:
 A single integer, representing the number of additional customer service executives that we would need to employ, to cover the call volume during peak times. If the current coverage is already sufficient, then print 0.
...
def cal_add_exe_need(cur, ts):
    ...
    for start, end in ts:
        start_dict[start] += 1
        end_dict[end] += 1
        ts_set.update([start, end])
    ts_list = sorted(list(ts_set))
    for each_ts in ts_list:
        exe_need += start_dict[each_ts]
        max_exe_need = max(max_exe_need, exe_need)
        exe_need -= end_dict[each_ts]
    return 0 if max_exe_need <= cur else max_exe_need - cur


if __name__ == "__main__":
    cur_executives = 1
    ts_set_list = [(1481122000, 1481122020), (1481122000, 1481122040), (1481122030, 1481122035)]
    print(cal_add_exe_need(cur_executives, ts_set_list))
```
