...
用[1,2,3]举例，第一次循环会生成三个分别以各元素开头的列表分别是[1], [2], [3]，之后进入递归，第二次循环还是遍历原列表中每个元素但是会判断当前元素是否已再temp_list中，添入未存在在列表中的元素后进入下一个递归循环，以此类推直到列表中元素个数等于原列表。

回溯便体现在向temp_list中增加一个元素之后进入递归，之后再pop出该元素，这样就可以实现分别以各元素为起始。
